...
#ifndef GAMEINSTALLER_TARFILE_H
#define GAMEINSTALLER_TARFILE_H
...
public:
    ...
    size_t GetTarSize() const;
    bool unzipToDir(const char* target);

    ~TarFile();
signals:
    void progressReady(int now, int count, QString filename);
private:
    FILE* file;
    size_t size;
